    ...
    if doOutputFile:
        ...
        for e in range(0,Nexp): # Do this loop for each experiment the user requested
            for t in range(0,Nmeas): # Do this loop for each measurement the user requested
                outfile.write(str(random.Exponential(rate))+" ") # Get and record a number from the random number distribution
        ...
        for e in range(0,Nexp):# Do this loop for each experiment the user requested
            for t in range(0,Nmeas):# Do this loop for each measurement the user requested
                print(random.Exponential(rate), end=' ') # Get and record a number from the random number distribution
            print(" ")